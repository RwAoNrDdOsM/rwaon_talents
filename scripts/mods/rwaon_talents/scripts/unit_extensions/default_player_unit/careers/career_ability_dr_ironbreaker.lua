...
mod:hook_origin(CareerAbilityDRIronbreaker, "_run_ability", function (self)
	...
	if buff_3 then
		buff_template_name_id_3 = NetworkLookup.buff_templates[buff_name_3]
	end
	...
	for i = 1, #targets, 1 do
		...
		if is_server then
			...
			if buff_template_name_id_3 then
				buff_extension:add_buff(buff_name_3, {
					attacker_unit = owner_unit
				})

				network_transmit:send_rpc_clients("rpc_add_buff", unit_object_id, buff_template_name_id_3, owner_unit_id, 0, false)
			end
		else
			...
			if buff_template_name_id_3 then
				network_transmit:send_rpc_server("rpc_add_buff", unit_object_id, buff_template_name_id_3, owner_unit_id, 0, true)
			end
		end
	end
