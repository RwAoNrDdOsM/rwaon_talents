...
local function trigger_player_killing_blow_ai_buffs(ai_unit, killing_blow)
    ...
    if Unit.alive(attacker_unit) and ScriptUnit.has_extension(attacker_unit, "buff_system") then
        ...
        buff_extension:trigger_procs("on_kill", killing_blow, breed_data, ai_unit)
    end
    ...
    if breed_data.boss then
        for i = 1, #player_and_bot_units, 1 do
            ...
            if buff_extension then
                buff_extension:trigger_procs("on_boss_killed", killing_blow, breed_data, ai_unit)
            end
        end
    ...
    if ping_extension then
        for i = 1, #player_and_bot_units, 1 do
            ...
            if buff_extension then
                buff_extension:trigger_procs("on_ping_target_killed", killing_blow, breed_data, ai_unit)
            end
        end
    end
end
